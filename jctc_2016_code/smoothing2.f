...
! construct the gaussian
      Ig = int(s/dx)
      write (*,'(a26,x, i2)') "# of points for gaussian =", Ig
      allocate( W(-Ig:Ig), WW(-Ig:Ig), Yfit(Nf) ) 
      do i=-Ig,Ig
        z = i*dx
        W(i) = gaussian(z,s)
      enddo

      ! original function smoothed with gaussian
...
! do a weighted moving average of the input data
      ...
      do i=1,Nf
         WW = W
         do m=-Ig,Ig
!            if (i+m>N .or. i+m<1) then
!               WW(m) = 0.
!               cycle
!            endif
!            Yfit(i) = Yfit(i) + W(m)*Fe(i+m)/ sum(WW)
            if (i+m>Nf) then
               ff = Fe(Nf)
            elseif (i+m<1) then
               ff = Fe(1)
            ...
            Yfit(i) = Yfit(i) + W(m)*ff/ sum(WW)
         enddo
      enddo

! write smoothed values 
      open (2,file='smoothed.dat')
      do i=1,Nf
         write (2,*) Xe(i), Yfit(i)
      enddo

! calculate the integral over smoothed values using simpson's rule
      print *, "smoothed integrand:  ", simpson( Yfit, dx, Nf)

! calculate the integral over unsmoothed data with Hermite interpolation
      a=X(1)
      b=X(N)
      rintegral = pchqa(N,X,F,D,a,b,ierr)       ! integration
      print *, "unsmoothed integrand:", rintegral

      end

      function simpson(Y,dx,N)
        real :: Y(N)
        simps = 1./3.*Y(1) + 1./3.*Y(N)
        do i=2,N-1
           if ( mod(i,2)==0 ) simps = simps + 4./3.*Y(i)
           if ( mod(i,2)==1 ) simps = simps + 2./3.*Y(i)
        enddo
        simpson = simps*dx
      end
