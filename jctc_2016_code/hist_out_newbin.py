...
for i in range(1, win_num):
      if i == 1:
            dictop[1] = range(200,355,5)
#      if i > 1:
#            dictop[i] = range(dictop[i-1][len(dictop[i-1])-1] - 5, dictop[i-1][len(dictop[i-1])-3] + 65, 5)
      if i == 2:
            dictop[i] = range(350,405,5)
      if i == 7:
            dictop[i] = range(600,900,5)
      if i == 3:
            dictop[i] = range(400,455,5)
      if i == 4:
            dictop[i] = range(450,505,5)
      if i == 5:
            dictop[i] = range(500,550,5)
      if i == 6:
            dictop[i] = range(550,600,25)
      print dictop[i]
...
#      matchval = ( ((dictop[i][int(len(dictop[i])-1)] + 25)/1000) - intercept ) / slope
      if i == 5:
            old = ((dictop[i][int(len(dictop[i])-1)] + 5)/1000)
      else:
            old = ((dictop[i][int(len(dictop[i])-1)] + 25)/1000)
      print old
      matchval = slope*old + intercept 
#      print (dictop[i][int(len(dictop[i])-1)] + 25)/1000, matchval, slope, intercept
      dict2[i][len(dict2[i])-1] = matchval 
''
corrtot = dict2[1]
print dict2[3]
